...
const watchListModel = require("../Models/bookingModel");
const userModel = require("../Models/userModel");


const gettingBookingDetails = async (req, res) => {
...
const addingToBookingDetails = async (req, res) => {
...
const updatingToBookingDetails = async (req, res) => {
  const { id } = req.params;
  const { emailString, details } = req.body;
  try {
    const user = await userModel.findOne({ email: emailString });
...
const deletingFromBookingDetails = async (req, res) => {
  const { id } = req.params;
  const { userId } = req.query;
  if (!mongoose.Types.ObjectId.isValid(id)) {
...
module.exports = { gettingBookingDetails, addingToBookingDetails, updatingToBookingDetails, deletingFromBookingDetails };